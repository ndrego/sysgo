...
func TestValueBinArithmetic(t *testing.T) {
	assert := assert.New(t)

	a, _ := NewValueString("1'b0")
	b, _ := NewValueString("1'b1")
	c, _ := NewValueString("1'bz")
	d, _ := NewValueString("1'bx")

	aPlusc, _ := NewValueString("2'bxx")
	assert.Equal(Hi, a.Binary("+", a).Binary("===", a).GetBit(0))
	assert.Equal(Hi, a.Binary("+", b).Binary("===", b).GetBit(0))
	assert.Equal(Hi, b.Binary("+", a).Binary("===", b).GetBit(0))
	assert.Equal(Hi, a.Binary("+", c).Binary("===", aPlusc).GetBit(0))
	assert.Equal(Hi, a.Binary("+", d).Binary("===", aPlusc).GetBit(0))
	assert.Equal(Hi, a.Binary("-", a).Binary("===", a).GetBit(0))
	assert.Equal(Hi, a.Binary("-", b).Binary("===", b).GetBit(0))
	assert.Equal(Hi, b.Binary("-", a).Binary("===", b).GetBit(0))
	assert.Equal(Hi, a.Binary("-", c).Binary("===", aPlusc).GetBit(0))
	...
	assert.Equal(Hi, a.Binary("*", a).Binary("===", a).GetBit(0))
	assert.Equal(Hi, a.Binary("*", b).Binary("===", a).GetBit(0))
	assert.Equal(Hi, b.Binary("*", a).Binary("===", a).GetBit(0))
	assert.Equal(Hi, b.Binary("*", b).Binary("===", b).GetBit(0))
	assert.Equal(Hi, a.Binary("*", c).Binary("===", d).GetBit(0))
	assert.Equal(Hi, a.Binary("*", d).Binary("===", d).GetBit(0))
	assert.Equal(Hi, a.Binary("/", a).Binary("===", d).GetBit(0))
	assert.Equal(Hi, a.Binary("/", b).Binary("===", a).GetBit(0))
	assert.Equal(Hi, b.Binary("/", a).Binary("===", d).GetBit(0))
	assert.Equal(Hi, b.Binary("/", b).Binary("===", b).GetBit(0))
	assert.Equal(Hi, a.Binary("/", c).Binary("===", d).GetBit(0))
	assert.Equal(Hi, c.Binary("/", a).Binary("===", d).GetBit(0))
	assert.Equal(Hi, a.Binary("/", d).Binary("===", d).GetBit(0))
	assert.Equal(Hi, d.Binary("/", a).Binary("===", d).GetBit(0))
	assert.Equal(Hi, a.Binary("%", a).Binary("===", d).GetBit(0))
	assert.Equal(Hi, a.Binary("%", b).Binary("===", a).GetBit(0))
	assert.Equal(Hi, b.Binary("%", a).Binary("===", d).GetBit(0))
	assert.Equal(Hi, b.Binary("%", b).Binary("===", a).GetBit(0))
	assert.Equal(Hi, a.Binary("%", c).Binary("===", d).GetBit(0))
	assert.Equal(Hi, a.Binary("%", d).Binary("===", d).GetBit(0))

	e, _ := NewValueString("2'b00")
	f, _ := NewValueString("2'b01")
	g, _ := NewValueString("5'h1a")
	h, _ := NewValueString("33'h1deadbeef")
	i, _ := NewValueString("64'hdeadbeef01234567")
	j, _ := NewValueString("64'hffff_ffff_ffff_ffff")
	k, _ := NewValueString("64'hffff_ffff_ffff_fffd")

	ePluse,  _ := NewValueString("3'b0")
	ePlusf,  _ := NewValueString("3'b1")
	fPlusg,  _ := NewValueString("6'h1b")
	gMinusf, _ := NewValueString("6'h19")
	fMinusg, _ := NewValueString("5'h7")
	gPlush,  _ := NewValueString("34'h1deadbf09")
	ePlusd,  _ := NewValueString("3'bxxx")
	eMinusd, _ := NewValueString("2'bxx")
	hMinusg, _ := NewValueString("34'h1deadbed5")
	hPlusi,  _ := NewValueString("65'hdeadbef0dfd10456")
	jMinusk, _ := NewValueString("64'h2")
	kMinusj, _ := NewValueString("64'hffff_ffff_ffff_fffe")
	gTimesg, _ := NewValueString("10'h2a4")
	gTimesh, _ := NewValueString("38'h309da56446")
	gTimesd, _ := NewValueString("6'hxx")
	hDivg,   _ := NewValueString("33'h126924e1")
	hModg,   _ := NewValueString("33'h15")

	assert.Equal(Hi, e.Binary("+", e).Binary("===", ePluse).GetBit(0))
	assert.Equal(Hi, e.Binary("+", f).Binary("===", ePlusf).GetBit(0))
	assert.Equal(Hi, f.Binary("+", g).Binary("===", fPlusg).GetBit(0))
	assert.Equal(Hi, g.Binary("+", h).Binary("===", gPlush).GetBit(0))
	assert.Equal(Hi, e.Binary("+", d).Binary("===", ePlusd).GetBit(0))
	assert.Equal(Hi, h.Binary("+", i).Binary("===", hPlusi).GetBit(0))
	assert.Equal(Hi, e.Binary("-", e).Binary("===", ePluse).GetBit(0))
	assert.Equal(Hi, f.Binary("-", e).Binary("===", ePlusf).GetBit(0))
	assert.Equal(Hi, g.Binary("-", f).Binary("===", gMinusf).GetBit(0))
	assert.Equal(Hi, f.Binary("-", g).Binary("===", fMinusg).GetBit(0))
	assert.Equal(Hi, h.Binary("-", g).Binary("===", hMinusg).GetBit(0))
	assert.Equal(Hi, e.Binary("-", d).Binary("===", eMinusd).GetBit(0))
	assert.Equal(Hi, j.Binary("-", k).Binary("===", jMinusk).GetBit(0))
	assert.Equal(Hi, k.Binary("-", j).Binary("===", kMinusj).GetBit(0))
	assert.Equal(Hi, e.Binary("*", e).Binary("===", e).GetBit(0))
	assert.Equal(Hi, e.Binary("*", a).Binary("===", e).GetBit(0))
	assert.Equal(Hi, a.Binary("*", e).Binary("===", e).GetBit(0))
	assert.Equal(Hi, e.Binary("*", f).Binary("===", e).GetBit(0))
	assert.Equal(Hi, g.Binary("*", f).Binary("===", g).GetBit(0))
	assert.Equal(Hi, g.Binary("*", g).Binary("===", gTimesg).GetBit(0))
	assert.Equal(Hi, g.Binary("*", h).Binary("===", gTimesh).GetBit(0))
	assert.Equal(Hi, g.Binary("*", d).Binary("===", gTimesd).GetBit(0))
	assert.Equal(Hi, e.Binary("/", f).Binary("===", e).GetBit(0))
	assert.Equal(Hi, f.Binary("/", e).Binary("===", eMinusd).GetBit(0))
	assert.Equal(Hi, f.Binary("/", b).Binary("===", f).GetBit(0))
	assert.Equal(Hi, h.Binary("/", g).Binary("===", hDivg).GetBit(0))	
	assert.Equal(Hi, g.Binary("/", h).Binary("===", e).GetBit(0))
	assert.Equal(Hi, h.Binary("%", g).Binary("===", hModg).GetBit(0))
	assert.Equal(Hi, e.Binary("%", f).Binary("===", e).GetBit(0))
	assert.Equal(Hi, f.Binary("%", e).Binary("===", eMinusd).GetBit(0))

	l, _ := NewValueString("65'h1ffff_ffff_ffff_ffff")
	m, _ := NewValueString("65'h1ffff_ffff_ffff_fffd")

	mPlusl,  _ := NewValueString("66'h3_ffff_ffff_ffff_fffc")
	mPlush,  _ := NewValueString("66'h2_0000_0001_dead_beec")
	mMinusl, _ := NewValueString("65'h1_ffff_ffff_ffff_fffe")
	mMinush, _ := NewValueString("65'h1_ffff_fffe_2152_410e")
	mTimesd, _ := NewValueString("66'hx_xxxx_xxxx_xxxx_xxxx")
	mTimesh, _ := NewValueString("98'h3_bd5b_7ddd_ffff_fffa_63f6_c333")
	mDivm,   _ := NewValueString("65'h1")
	mDivd,   _ := NewValueString("65'hx_xxxx_xxxx_xxxx_xxxx")
	mDivi,   _ := NewValueString("65'h2")
	mModh,   _ := NewValueString("65'h1_9671_54dd")

	assert.Equal(Hi, m.Binary("+", l).Binary("===", mPlusl).GetBit(0))
	assert.Equal(Hi, m.Binary("+", h).Binary("===", mPlush).GetBit(0))
	assert.Equal(Hi, l.Binary("+", a).Binary("===", l).GetBit(0))	
	assert.Equal(Hi, m.Binary("-", l).Binary("===", mMinusl).GetBit(0))
	assert.Equal(Hi, m.Binary("-", h).Binary("===", mMinush).GetBit(0))
	assert.Equal(Hi, l.Binary("-", l).Binary("===", a).GetBit(0))
	assert.Equal(Hi, m.Binary("*", a).Binary("===", a).GetBit(0))
	assert.Equal(Hi, m.Binary("*", d).Binary("===", mTimesd).GetBit(0))
	assert.Equal(Hi, m.Binary("*", h).Binary("===", mTimesh).GetBit(0))
	assert.Equal(Hi, m.Binary("/", m).Binary("===", mDivm).GetBit(0))
	assert.Equal(Hi, m.Binary("/", d).Binary("===", mDivd).GetBit(0))
	assert.Equal(Hi, l.Binary("/", m).Binary("===", b).GetBit(0))
	assert.Equal(Hi, m.Binary("/", l).Binary("===", a).GetBit(0))
	assert.Equal(Hi, m.Binary("/", i).Binary("===", mDivi).GetBit(0))
	assert.Equal(Hi, m.Binary("%", l).Binary("===", m).GetBit(0))
	assert.Equal(Hi, l.Binary("%", m).Binary("===", mDivi).GetBit(0))
	assert.Equal(Hi, m.Binary("%", h).Binary("===", mModh).GetBit(0))
}
